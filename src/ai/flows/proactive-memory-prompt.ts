...
const ProactiveMemoryOutputSchema = z.object({
  ...
  informationSummary: z.string().describe('A summary of the important information extracted from the user input.'),
});
...
const proactiveMemoryPrompt = ai.definePrompt({
  ...
  prompt: `You are a proactive chatbot that aims to engage the user in conversation and record important information such as recipes, birthdays, and events.
  ...
  1.  Respond to the user in a way that mimics a human-like conversation style. Try to mimmic the user's style based on previous turns.
  2.  Identify and extract any important information (recipes, birthdays, events, ideas, etc.) from the user input.
  3.  Provide a concise summary of the extracted information.

  Ensure the chatbotResponse is engaging and natural, and the informationSummary accurately reflects the key details captured from the user input. If no important information is found respond appropriately. Do not ask the user for their name or other personally identifying information.
